...
public class FightPageFormController extends AbstractController {
	...
	protected ModelAndView handleRequestInternal(HttpServletRequest request,
		...
		model.put("fighter1_health",
				String.valueOf(game.getFighterFirst().getHealth()));
		model.put("fighter2_nickName", game.getFighterSecond().getNickName());
		model.put("fighter2_avatar", game.getFighterSecond().getAvatar());
		model.put("fighter2_health",
				String.valueOf(game.getFighterSecond().getHealth()));
		log.info("FIGHT PAGE CREATED IN GAME = " + game.getName() + " FOR "
				+ user.getEmail());
